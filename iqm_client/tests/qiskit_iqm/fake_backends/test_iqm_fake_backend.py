...
@pytest.fixture
def backend(linear_3q_static_architecture, create_3q_error_profile):
    return IQMFakeBackend(linear_3q_static_architecture, create_3q_error_profile())


def test_fake_backend_with_incomplete_t1s(linear_3q_static_architecture, create_3q_error_profile):
    """Test that IQMFakeBackend construction fails if T1 times are not provided for all qubits"""
    with pytest.raises(ValueError, match=r"t1s is missing loci: \{'QB2'\}"):
        error_profile = create_3q_error_profile(t1s={"QB1": 2000, "QB3": 2000})
        IQMFakeBackend(linear_3q_static_architecture, error_profile)


def test_fake_backend_with_incomplete_t2s(linear_3q_static_architecture, create_3q_error_profile):
    """Test that IQMFakeBackend construction fails if T2 times are not provided for all qubits"""
    with pytest.raises(ValueError, match=r"t2s is missing loci: \{'QB3'\}"):
        error_profile = create_3q_error_profile(t2s={"QB1": 2000, "QB2": 2000})
        IQMFakeBackend(linear_3q_static_architecture, error_profile)
...
def test_fake_backend_with_single_qubit_gate_depolarizing_errors_qubits_not_matching_quantum_architecture(
    ...
    with pytest.raises(
        ValueError, match=r"single_qubit_gate_depolarizing_error_parameters\['prx'\] is missing loci: \{'QB3'\}"
    ):
...
def test_fake_backend_with_single_qubit_gate_depolarizing_errors_more_qubits_than_in_quantum_architecture(
    ...
    with pytest.raises(
        ValueError, match=r"single_qubit_gate_depolarizing_error_parameters\['prx'\] has unknown loci: \{'QB4'\}"
    ):
        error_profile = create_3q_error_profile(
...
def test_fake_backend_with_two_qubit_gate_depolarizing_errors_couplings_not_matching_quantum_architecture(
    ...
    with pytest.raises(
        ValueError,
        match=r"two_qubit_gate_depolarizing_error_parameters\['cz'\] is missing loci",
    ):
        error_profile = create_3q_error_profile(
...
def test_fake_backend_with_two_qubit_gate_depolarizing_errors_more_couplings_than_in_quantum_architecture(
    ...
    with pytest.raises(
        ValueError,
        match=r"two_qubit_gate_depolarizing_error_parameters\['cz'\] has unknown loci",
    ):
        error_profile = create_3q_error_profile(
...
def test_fake_backend_not_matching_quantum_architecture(
    ...
    with pytest.raises(
        ValueError,
        match=f"Gate 'wrong' in {param_name} is unknown",
    ):
        error_profile = create_3q_error_profile(**{param_name: param_value})
        IQMFakeBackend(linear_3q_static_architecture, error_profile)


def test_error_profile(linear_3q_static_architecture, create_3q_error_profile):
    err_profile = create_3q_error_profile()
    backend = IQMFakeBackend(linear_3q_static_architecture, err_profile)
...
def test_copy_with_error_profile(linear_3q_static_architecture, create_3q_error_profile):
    err_profile = create_3q_error_profile()
    backend = IQMFakeBackend(linear_3q_static_architecture, err_profile)
...
def test_fake_backend_with_readout_errors_more_qubits_than_in_quantum_architecture(
    ...
    other qubits than specified in the quantum architecture"""
    with pytest.raises(ValueError, match=r"readout_errors has unknown loci: \{'QB4'\}"):
        error_profile = create_3q_error_profile(
            readout_errors={
                "QB1": {"0": 0.02, "1": 0.03},
                "QB2": {"0": 0.02, "1": 0.03},
...
def test_validate_compatible_architecture(adonis_static_architecture, linear_3q_static_architecture):
    backend = IQMFakeAdonis()
    assert backend.validate_compatible_architecture(adonis_static_architecture) is True
    assert backend.validate_compatible_architecture(linear_3q_static_architecture) is False
