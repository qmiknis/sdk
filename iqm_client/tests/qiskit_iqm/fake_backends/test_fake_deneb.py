...
@pytest.fixture
def mock_iqm_fake_deneb(mock_iqm_backend_init):
...
def test_move_gate_sandwich_interrupted_with_single_qubit_gate(mock_iqm_fake_deneb):
    backend = mock_iqm_fake_deneb
    no_qubits = 1
...
def test_simulate_ghz_circuit_with_iqm_fake_deneb_noise_model(mock_iqm_fake_deneb):
    backend = mock_iqm_fake_deneb
    no_qubits = 6
    comp_r = QuantumRegister(1, "comp_r")  # Computational resonator
    q = QuantumRegister(no_qubits, "q")  # Qubits
...
def test_transpile_to_IQM_for_ghz_with_fake_deneb_noise_model():
    ...
    transpiled_qc = transpile_to_IQM(
        qc, backend=backend, optimize_single_qubits=False, existing_moves_handling=ExistingMoveHandlingOptions.REMOVE
    )
...
def test_transpiling_works_but_backend_run_doesnt_with_unsupported_gates(mock_iqm_fake_deneb):
    backend = mock_iqm_fake_deneb
    num_qb = 1
    qc_list = []
    for _ in range(4):
    ...
    for qc in qc_list:
        backend.run(
            transpile_to_IQM(qc, backend=backend, existing_moves_handling=ExistingMoveHandlingOptions.REMOVE),
            shots=1000,
        )
