...
from .utils import capture_submitted_circuits
...
def backend(request):
    ...
    return get_mocked_backend(request.getfixturevalue(request.param), request)
    ...
    list(
        product(
            ...
            [
                "transpiled_to_IQM",
                "qiskit_integration",
            ],  # "qiskit" can be used for debugging
        )
    ),
    indirect=["backend"],
)
class TestTranspilation:
    ...
    @pytest.mark.xfail(
        reason="There seems to be an issues where the transpiled circuit is not always semantically "
        "equivalent to the original one. SW-1999"
    )
    ...
    def test_transpiled_circuit_keeps_layout(self):
        ...
        layout = [
            int(qb)
            for qb in RNG.choice(
                range(self.backend.num_qubits),
                self.original_circuit.num_qubits,
                replace=False,
            )
        ]
...
def test_transpiling_with_unused_qubits(star_architecture, request):
    """Test that transpile_to_IQM can handle unused qubits in the circuit."""

    backend = get_mocked_backend(star_architecture, request)
    n_qubits = 3
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("star_architecture", ["QB5", "QB3", "QB1", "CR1"]),
    ],
    indirect=["backend"],
)
def test_transpiling_with_restricted_qubits(backend, restriction):
    ...
    for restricted in [restriction, restriction_idxs]:
        ...
        validate_circuit(transpiled_circuit, backend, qubit_index_to_name=dict(enumerate(restriction)))
        assert transpiled_circuit.num_qubits == len(restricted)
        if hasattr(backend, "client"):
            # Check that the run doesn't fail.
            capture_submitted_circuits()
            backend.run(transpiled_circuit, shots=1, qubit_index_to_name=dict(enumerate(restriction)))


def test_transpile_empty_optimized_circuit(star_architecture, request):
...
def test_get_scheduling_method(remove_final_rzs, ignore_barriers, existing_moves_handling):
    ...
    elif remove_final_rzs is False and ignore_barriers is True:
...
def test_transpile_to_IQM_flags(
    ...
    if existing_moves_handling and remove_final_rzs is False and perform_move_routing and optimize_single_qubits:
        with pytest.raises(ValueError, match="Existing Move handling options are not compatible"):
            transpile_to_IQM(
                QuantumCircuit(),
                get_mocked_backend(star_architecture, request),
        ...
        if perform_move_routing is False and optimize_single_qubits is False:
            assert isinstance(
                plugin,
                qiskit.transpiler.preset_passmanagers.builtin_plugins.DefaultSchedulingPassManager,
            )
