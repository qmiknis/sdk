...
class ScheduleBuilder:
                    ...
                    if self.op_table[op].factorizable and op in self._cache and impl in self._cache[op]:
                        ...
                        for cached_locus in list(self._cache[op][impl]):
                            if locus_component in set(cached_locus):
                                _fix_cache(op, impl, cached_locus)
    ...
    def get_implementation(
        ...
        *,
        ...
        The returned implementation may use a permuted ``locus`` (and the calibration data
        ...
        * If the operation is not symmetric (e.g. CX), different ``locus`` orders correspond to different
          operations, and the locus order many not be changed.

        * If the operation is symmetric (e.g. CZ), all ``locus`` orders correspond to the same logical operation.
          However, symmetric operations may still have non-symmetric implementations.
          ...
          * Otherwise we check all permutations of ``locus``, and pick the first one that
        ...
        if op.factorizable:
            ...
            if len(locus) > 1:
                ...
                def aggregator_callback() -> GateImplementation:
                    return impl_class.construct_aggregator(
                        parent=op,
                        name=impl_name,
                        locus=locus,
                        sub_implementations={
    ...
    def _pick_implementation_and_locus(
        ...
        *,
        ...
        """Find an implementation and locus for the given quantum operation compatible
        with both the calibration data and the implementation and locus requested by the caller.

        For symmetric ``op``, may change the locus order.
        ...
        # see if the locus order needs to be modified
        if not impl_class.needs_calibration() or not op.symmetric:
            # If no cal data is needed, any locus is ok.
            # For non-symmetric ops the locus cannot be changed.
            # FIXME This is wrong for compositegates, see SW-1016
            return impl_name, locus
